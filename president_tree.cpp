...
Node* modify(int l,int r,int x,Node* nroot)
{
    ...
    if(l>=r)return ptr;
    int mid=(l+r)>>1;
    if(x<=mid)ptr->lc=modify(l,mid,x,nroot->lc);
    else ptr->rc=modify(mid+1,r,x,nroot->rc);
    return ptr;
}
int query(int l,int r,int q,Node* nl,Node* nr)
{
    ...
    int mid=(l+r)>>1;
    if(q<=x)return query(l,mid,q,nl->lc,nr->lc);
    else return query(mid+1,r,q-x,nl->rc,nr->rc);
}
...
int main()
{
    ...
    _Root.push_back(NULL);
    _Root[0]=build(1,len);
    for(int i=1;i<=n;++i)
    {
        int p=lower_bound(b+1, b+len+1,a[i])-b;
        _Root.push_back(modify(1,len,p,_Root[i-1]));
    }
    for(int i=0;i<m;++i)
    {
        int l,r,k;
        scanf("%d%d%d", &l, &r, &k);
        int ans=query(1,len,k,_Root[l-1],_Root[r]);
        printf("%d\n", b[ans]);
    }
}