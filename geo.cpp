...
inline int ConvexHull(Point A[], int n, Point Ans[])
{
	...
	for (int i = 1; i <= n; ++i)
	{
		while (cnt > 1 && Cross(Ans[cnt] - Ans[cnt - 1], A[i] - Ans[cnt - 1]) <= 0)
			cnt--;
		Ans[++cnt] = A[i];
	}
	int k = cnt;
	for (int i = n - 1; i >= 1; --i)
	{
		while (cnt > k && Cross(Ans[cnt] - Ans[cnt - 1], A[i] - Ans[cnt - 1]) <= 0)
			cnt--;
		Ans[++cnt] = A[i];
	}
	return cnt - 1;
	...
	for (int u = 1, v = 2; u <= n; u++)
	{
		while (true)
		{
			...
			if (val <= 0)
			{
				...
				if (!val)
					ans = std::max(ans, Length(Poly[u] - Poly[v + 1]));
				break;
			}
			v = v % n + 1;
		}
...
int HpI(Line L[], int n, Point Poly[])
{
	...
	for (int i = 2; i <= n; ++i)
	{
		while (f < l && !OnLeft(L[i], p[l - 1]))
			l--; // 如果原来的交点在新加入直线的右边，就没用了。
		while (f < l && !OnLeft(L[i], p[f]))
			f++;
		q[++l] = L[i];
		...
		if (f < l)
			p[l - 1] = LI(q[l], q[l - 1]); // 新的交点
	}
	while (f < l && !OnLeft(q[f], p[l - 1]))
		l--;
	if (l - f <= 1)
		return 0;
	p[l] = LI(q[l], q[f]);
